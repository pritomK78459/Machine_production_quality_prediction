...
def train_decision_tree_regressor(X_train:pd.DataFrame, y_train:pd.Series):
    ...
    # this function will train a Random Forest Regressor on the train features and target

    RandomForest = RandomForestRegressor() # initialize the regressor
    ...
    results = RandomForest.fit(X_train, y_train) # train the regressor
...
if __name__ == '__main__':
    ...
    parser.set_defaults(features='datasets/preprocessed_datasets/preprocessed_features.csv',
                        ...
                        output='Models/RandomForestBest.sav',
                        save=True)
    ...
    decomposed_features = pca.fit_transform(scaled_features)
